...
public class UserShopDetailProductionPerm extends Fragment {
    private RecyclerView mRecyclerView;
    private RecyclerView.LayoutManager mLayoutManager;
    private HaircolorRecyclerviewAdapter mAdapter;
    private UserShopDetail userShopDetail;
    private List<HairStyle> premList;

    OkHttpClient okHttpClient = new OkHttpClient();

    private static final int PERM = 1;

    public UserShopDetailProductionPerm(UserShopDetail userShopDetail) {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        ...
        //加载选项卡对应的选项页面
        View view = inflater.inflate(R.layout.fragment_user_shop_detail_production_perm,container,false);

        //创建静态数据
        PrepareHairStylePicture hairStyles = new PrepareHairStylePicture();
        mRecyclerView = view.findViewById(R.id.fragment_shop_detail_perm_recyclerview);
        //设置布局管理器为2列，纵向
        selectCut("烫发",String.valueOf(userShopDetail.getShopId()));
        return view;
    }

    private void initAdapter() {
        mLayoutManager = new StaggeredGridLayoutManager(2, StaggeredGridLayoutManager.VERTICAL);
        mAdapter = new HaircolorRecyclerviewAdapter(getActivity().getApplicationContext(), R.layout.item_user_main_haircolor_recyclerview,premList);
