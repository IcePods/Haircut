...
public abstract class NineImgView extends ViewGroup {
    ...
    public NineImgView(Context context, AttributeSet atts){
        super(context, atts);
        TypedArray typedArray = context.obtainStyledAttributes(atts, R.styleable.NineImgViewLayout);
    ...
    private void refresh() {
        removeAllViews();
        int size = getListSize(ImageUrlList);
        if (size > 0) {
            setVisibility(VISIBLE);
        } else {
            setVisibility(GONE);
    ...
    private ImageView createImageView(final int i, final String url) {
        ...
        imageView.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                onClickImage(i, url, ImageUrlList);
            }
        });
        return imageView;
    }
    ...
    private void layoutImageView(ImageView imageView, int i, String url, boolean showNumFlag) {
        ...
        if (showNumFlag) {//添加超过最大显示数量的文本
            int overCount = getListSize(ImageUrlList) - MAX_COUNT;
            if (overCount > 0) {
                float textSize = 30;
